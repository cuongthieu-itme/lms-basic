    ...
    <div class="p-4 space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        >
          ...
          <p class="text-sm text-gray-600">Bài quiz: {{ question.quiz_title }}</p>
          <div class="flex justify-end gap-2 text-sm">
                          <Link :href="route('question.edit', question.id)" class="text-blue-600 hover:underline">Sửa</Link>
                            <button @click="confirmDelete(question.id)" class="text-red-600 hover:underline">Xóa</button>
          </div>
        </div>
      </div>
    </div>
